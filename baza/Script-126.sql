...
FROM instruktor
WHERE 1=1  and username = 'aca' and pass = 'a';
...
alter table teorijskicas drop constraint `PRIMARY`;
alter table teorijskicas add constraint pk_teorijskiCas primary key (rbr_teor_cas);
...
insert into teorijskicas (id_instr, id_grupa, dt, rbr_teor_cas, tema, usc, dtc, usm, dtm)
value (:p_id_instr, :p_id_grupa, current_date, 
        (select coalesce(max(t.rbr_teor_cas),0)+1 
        from teorijskicas t 
        where t.id_instr = :p_id_instr and t.id_grupa = :p_id_grupa),
        'Neka tema',
        'aca',
        current_timestamp,
        null, null)
...
alter table tt add column datum date

alter table tt add column datumVreme timestamp

insert into tt() values(1, 'text', '2021-11-13', '2021-11-13 12:24:20')
